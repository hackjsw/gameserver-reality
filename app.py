#!/bin/bash
# Unreal Engine 5.3 Pro 部署与优化工具 (V3.8 - Pterodactyl 完美伪装版)
# 功能：环境适配、Reality 协议、控制台链接隐藏、消除 Xray 输出特征
...
start_process() {
    ...
    echo -e "Deployment metadata secured in: \033[33m/home/container/Project_Access.log\033[0m"
    echo -e "--------------------------------------------------"

    # 核心修改：静默启动 Xray 并接管控制台输出
    # 将输出全部重定向到 /dev/null
    "$BIN_PATH" run -c "$CFG_PATH" > /dev/null 2>&1 &
    XRAY_PID=$!

    # 退出保护：如果脚本被杀，同时杀掉 Xray 进程
    trap "kill $XRAY_PID; exit" INT TERM

    # 持续伪装循环，防止面板判定 Offline
    while true; do
